...
class GraphDataset(Dataset):
    def __init__(self, ukb_filtered, root, data_field, mapping=None, oversample=False, transform=None, pre_transform=None, pre_filter=None):
    ...
    def process(self, k_degree=10):
        ...
        for raw_path in self.raw_paths:
            ...
            enc_label = self.ukb_filtered.iloc[i, 1]  
            ...
            # print(f"label: {label}, enc_label: {enc_label}", data.y)

            torch.save(data, os.path.join(self.processed_dir, str(i) + '.pt'))
            i += 1
    ...
    def get(self, idx):
        data = torch.load(os.path.join(self.processed_dir, str(idx) + '.pt'))
        enc_label = data.y[0] # return label as int
        return data, enc_label
...
if __name__ == "__main__":
    ...
    ds = GraphDataset('../data/2_class_balanced_ukb_filtered_25753_harir_mh_upto69.csv','../data/fetched/25751', '25751', mapping, oversample=True)
