...
from dataloaders.dataloader import balanced_random_split_v2
...
def train(dataset, seed, data_dir, udi, config=None, oversampling=False):
    ...
    ds = GraphDataset('data/balanced_sex_classification.csv', data_dir, udi, mapping=None)
    with wandb.init(config=config):
        config = wandb.config

        ds.process(config.k_degree)
        total_size = len(ds)
        ...
        train_d, val_d, test_d = balanced_random_split_v2(dataset, [train_size, val_size, test_size], num_classes=n_classes)
        ...
        model = GCN(55, n_classes, config.k_order, config.dropout).to(DEVICE)
        ...
        for epoch in range(config["epochs"]):
            ...
            train_metrics, _ = evaluate_model(model, train_loader, num_classes=n_classes)
            train_acc[epoch] = train_metrics["accuracy"]

            val_metrics, val_loss[epoch] = evaluate_model(model, val_loader, num_classes=n_classes)
        ...
        # test best model
        test_metrics, _ = evaluate_model(best_model, test_loader, num_classes=n_classes)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project=f"GCN_{sweep_config['name']}_{udi}")
