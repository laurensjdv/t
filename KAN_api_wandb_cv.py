...
def run_experiment(ds, seed, data_dir, udi, config=None):
    ...
    for n_fold, (train_val, test) in enumerate(skf.split(labels, labels)):
        ...
        model = KAN(width=dims, grid=grid_size, seed=seed, device=DEVICE, symbolic_enabled=False, bias_trainable=True)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project=f"{dataset_name}_KANapi_{sweep_config['name']}")
