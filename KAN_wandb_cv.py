...
def KAN_test(model, loader, val_dataset, batch_size):
    pred = []
    label = []
    loss_all = 0
    for data in loader:
...
def run_experiment(ds, seed, data_dir, udi, config=None):
    ...
    for n_fold, (train_val, test) in enumerate(skf.split(labels, labels)):
        ...
        test_auc, test_acc, _ = KAN_test(best_model, test_loader, test_dataset, batch_size)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project=f"{dataset_name}_KAN_{sweep_config['name']}_{udi}")
