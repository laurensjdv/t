...
def MLP_test(model, loader, val_dataset, batch_size):
    ...
    for data in loader:
        ...
        loss = func.cross_entropy(output, labels)
        loss_all += batch_size * loss.item()
        predicted = func.softmax(output, dim=1).max(dim=1)[1]
...
def run_experiment(ds, seed, data_dir, udi, config=None):
    ...
    run_name = run.name
    torch.save(best_model.state_dict(), f"models/saved/MLP/{run_name}.pth")
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project=f"{dataset_name}_MLP")
