...
from dataloaders.dataloader import GraphDataset
...
k_degree = 10
k_order = 5
learning_rate = 0.00008769
weight_decay = 0.001

skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=99)
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# dataset = 'data/csv/smith_data_shuffled.csv'
# dataset = 'data/csv/gal_data_shuffled.csv'
# dataset = 'data/csv/matched_rds_over_8.csv'
# dataset = 'data/matched_pure_rds_over_8.csv'
# dataset = 'data/matched_pure_smith_single_ep.csv'
# dataset = 'data/fully_severe_balanced_smith.csv'
# dataset = 'data/matched_smith_moderate_rds_over_8.csv'
...
# dataset = FCMatrixDataset(dataset, 'data/fetched/25751/raw', '25751', None)
dataset = GraphDataset(ds, 'data/fetched/25751', '25751')
dataset.process(k_degree=k_degree)

print(len(dataset))
for n_fold, (train_val, test) in enumerate(skf.split(labels, labels)):
    
    # model = MLP(1485, hidden_dims, 2, dropout).to(device)
    model = GCN(dataset.num_features, 2, k_order=k_order, dropout=dropout).to(device)
